...
if (typeof GemiusHitcolConnector == "undefined") {
	GemiusHitcolConnector = function(is_gde) {
		...
		if (this.use_globals) {
			...
			this.hc = (typeof gemius_hitcollector === 'string')?gemius_hitcollector:(typeof pp_gemius_hitcollector === 'string')?pp_gemius_hitcollector:'gars.hit.gemius.pl';
			...
			this.get_title = (typeof gemius_get_title === 'boolean')?gemius_get_title:(typeof pp_gemius_get_title === 'boolean')?pp_gemius_get_title:false;
		} else {
			this.ssl = (document.location && document.location.protocol && document.location.protocol=='https:')?1:0;
			this.hc = 'gars.hit.gemius.pl';
			this.dnt = 0;
			this.use_cmp = 0;
...
var check_same_origin = function(ref, page_location) {
	...
	return null;
		...
		this.init_params = function(p) {
			...
			this.get_title = (typeof p['get_title'] === 'boolean')?p['get_title']:this.get_title;
		};
		...
		this.sendhit = function(robj,nr,consent,lsadd) {
			...
			params += '&fpdata=' + (consent?this.getfpdataencoded():'-NOCONSENT');
			if (this.tfp.fpdata != '') {
		...
		this.unloadhit = function(robj,nr,consent) {
			...
			params += '&fpdata='+((this.waiting_for_fpdata==0 && consent)?this.getfpdataencoded():'-UNLOAD');
		...
		this.getrawfpdata = function() {
			return this.fpdata.split('|')[0];
		};
		this.getconsentfpdata = function() {
			return this.has_consent?this.getrawfpdata():'-NOCONSENT';
		};
		this.getfpdataencoded = function() {
			if (typeof encodeURIComponent != 'undefined') {
				return encodeURIComponent(this.fpdata);
			} else {
				return this.getrawfpdata();
			}
		}
		this.getconsentlsdata = function() {
			return this.has_consent?this.lsdata:'-NOCONSENT';
		};
		var _get_fpdata_ts = function(fpdata) {
			var arr = fpdata.split('|');
			return (arr.length >= 2) ? parseInt(arr[1]) * 1000 : 0;
		};
		_get_fpdata_ts = this.bind(_get_fpdata_ts);
		this.parse_fpdata = function() {
			var arr = this.fpdata.split('|');
			var segments = (arr.length >= 3) ? parseInt(arr[2]) : 0;
			if (!this.fpcdomain && segments>1) {
				var domain = new String(document.location.hostname);
				var arr2 = domain.split('.');
				if (arr2.length >= segments) {
					this.fpcdomain = arr2.slice(arr2.length-segments, arr2.length).join('.');
				}
			}
		};
		...
		this.restore_fpcookie = function() {
			if (typeof gemius_cookie=='object' && gemius_cookie!=null && this.fpdata!='' && this.fpdata[0]!='-' && this.fpcdomain!='') {
				var fpcookie = gemius_cookie.get_on_state(this.fpcookie_name, this.fpdata_sec_state);
				if (fpcookie.primary=='' || fpcookie.primary[0]=='-') {
					gemius_cookie.set(this.fpcookie_name, this.fpdata, this.cookie_expire, this.fpcdomain, this.fpdata_sec_state, false);
					var fpcap = this.fpcap[this.fpdata_sec_state];
					if (fpcap!='' && fpcap[0]!='-') {
						gemius_cookie.set(this.capcookie_name, fpcap, this.cookie_expire, this.fpcdomain, this.fpdata_sec_state, false);
					}
				}
				if (this.fpdata_secondary!='' && this.fpdata_secondary[0]!='-' && (fpcookie.secondary=='' || fpcookie.secondary[0]=='-')) {
					gemius_cookie.set(this.fpcookie_name, this.fpdata_secondary, this.cookie_expire, this.fpcdomain, "sec", false);
					var fpcap = this.fpcap["sec"];
					if (fpcap!='' && fpcap[0]!='-') {
						gemius_cookie.set(this.capcookie_name, fpcap, this.cookie_expire, this.fpcdomain, "sec", false);
					}
				}
			}
		};
		...
		this.fpcookie_secondary_needs_reset = function() {
			return (this.gdprforgetts>0 && this.fpdata_secondary!='' && this.fpdata_secondary[0]!='-' && _get_fpdata_ts(this.fpdata_secondary)<this.gdprforgetts);
		};
		this.setfpcookie = function() {
			var reset_secondary = this.fpcookie_secondary_needs_reset();
			this.fpdbg = gemius_cookie.set(this.fpcookie_name, this.fpdata, this.cookie_expire, this.fpcdomain, this.fpdata_sec_state, reset_secondary);
			if (reset_secondary) {
				this.fpdata_secondary = this.fpdata;
			}
			if (this.is_gde) {
				this.sync_gde_params();
			}
...
// cmp_utils.js 
		...
		this.parameters = function(href) {
			...
			if (this.get_title) {
				this.inner_dict['title'] = document.title || '';
			}
			return url;
		};
		...
		this.inner_params = function() {
			if (typeof encodeURIComponent != 'undefined') {
				...
				if (this.inner_dict['title']) {
					inner += '|_title=' + this.inner_dict['title'].substring(0,100).replace(/\|/g, '-');
				}
		...
		this.tfp_fix_url = function() {
			var url = (this.hssl?'https://':'http://')+this.hc+'/_';
			var d = new Date().getTime();
			url += d + "/redot.gif?fpdata=" + this.getfpdataencoded();
...
// chints_params.js 
	...
	return params;
		...
		this.init = function(params) {
			...
			this.send_bfview();
			setInterval(this.bind(this.restore_fpcookie),1000);
		};
		this.start = function() {
			if (this.started) {
				return;
